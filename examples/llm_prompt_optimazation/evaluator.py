...
from examples.llm_prompt_optimazation.singleevaluator import evaluate_single_example

TASK_MODEL_NAME = "mistralai/mistral-small-3.2"
TASK_MODEL_URL = "http://192.168.176.251:1234/v1"
TASK_MODEL_API_KEY = "your_api_key_here"

SAMPLE_SIZE = 5  # Number of samples to use for evaluation
...
def evaluate(prompt_path):
    ...
    try:
        ...
        for example in tqdm(ds_sample, desc="Evaluating examples", unit="example"):
            total_examples += 1
            total_score = evaluate_single_example(example, individual_scores, prompt, test_model, total_score)
